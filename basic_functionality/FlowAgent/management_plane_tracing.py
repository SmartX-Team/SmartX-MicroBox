...
# create raw socket, bind it to eno1
# attach bpf program to socket created
BPF.attach_raw_socket(function_ip_filter, "eno1")
...
while 1:
    ...
    if (str(gettime)=='0'):
     gettime='00'
    filename = "/opt/IOVisor-Data/"+BoxName+"-mc-" + time.strftime("%Y-%m-%d-%H") + "-" +str(gettime)
    f = open(filename, "a")
    f.write("%s\n" % MESSAGE)
    f.close
#    time.sleep(1)
#    subprocess.call(['/opt/MultiView-Dependencies/multi-view-flowcentric-integration.sh'])
