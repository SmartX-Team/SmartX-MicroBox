...
def check_ping_vc():
        with open('/tmp/SmartX-MicroBox/basic_functionality/Measurement/docker/micro-box-config.yaml') as f:
                content = f.readlines()
...
#         print 'latency_avg:' + pingstatus[0] + ' latency_min:' + pingstatus[1]+ ' latency_max:' + pingstatus$
            else:
...
filepath="ping_latency.log"
filepath_throughput="throughput.log"
filepath_missed_collection="measurement_collection_missed.log"
filepath_failures="measurement_failures.log"
#f = open("ping_latency.log", "a")
MESSAGE="Connection"
loop='true'

def no_connection():
     print "Not connected with VCenter"
     if "smartx-microbox_ping_latency"=="smartx-microbox_ping_latency":
...
#		 if(os.stat("ping_latency.log").st_size > 0):
		 cnt=0
...
##############################################################################TCP/UDP####################################################################################################
		...
		elif "smartx-microbox_ping_latency"=="tcp_udp":
	         print "******SENDING THROUGHPUT DAILY REPORT THROUGH KAFKA*********"
