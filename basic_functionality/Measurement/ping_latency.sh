...
#	cat  /opt/measurement/micro-box-config.yaml | while read line
	while read line
        do
#         TO_BOX="cut -d ":" -f 2 <<< $line1
	...
	python /opt/measurement/daily_report.py "smartx-microbox_ping_latency" "$BOX_HOSTNAME" "$ping_grid" "$latency_grid" "$vcenter_check"
}
