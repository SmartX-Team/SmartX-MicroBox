...
public class get_vcenter_elasticsearch_aggregate extends TimerTask {
	...
	private String ESindex = "pers_vcenter_liveliness_aggregate_es";
	private Elasticsearch_Connector ESConnector;
	public get_vcenter_elasticsearch_aggregate(MongoDB_Connector mongoConnector2,Elasticsearch_Connector eSConnector2) {
...
/////Getting data from previous day//////
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			for( int i = 0; i < count_site_yaml; i++)
			{
				...
				System.out.println("sites_name[i]:count_oneday_vcenter_SmartX_Box"+sites_name[i]+":"+count_oneday_vcenter_SmartX_Box);
			...
			document_vcenter_status.put("timestamp",   simpleDateFormat.format(yesterday()/*timeStamp_file*/));
			...
			for( int i = 0; i < count_site_yaml; i++)
			{
				...
				else
				{
					...
					//put liveliness of micro-box with vcenter in the elastic search 
					ESConnector.insertLivelinessVcenter(ESindex,/*simpleDateFormat.format*/(yesterday()),sites_name[i].replace("-", "_"),Float.toString(f1));	
				}
				
				System.out.println("document_vcenter_status"+document_vcenter_status.getString(sites_name[i]));
			}
			...
			//out.print(simpleDateFormatNoTime.format(yesterday())+" "+document_vcenter_status.get("SmartX-Box-SOURCE")+" ");
			for( int i = 0; i < sites_name_working.length; i++)
			{
				
				//out.print(document_vcenter_status.getString(sites_name_working[i])+"%" + " ");
				if(!document_vcenter_status.getString(sites_name_working[i]).contains("-"))
				{
					ESConnector.insertVcenterDataAggregate(ESindex,/*simpleDateFormat.format*/timestamp, sites_name_working[i].replace("-", "_"),Float.valueOf(document_vcenter_status.getString(sites_name_working[i])));	
				}
			}
			//out.println("");
