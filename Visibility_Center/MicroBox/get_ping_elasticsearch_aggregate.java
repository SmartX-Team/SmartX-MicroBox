...
public class get_ping_elasticsearch_aggregate  extends TimerTask {
	...
	private String ESindex = "pers_ping_aggregate_es";
	private String ESindex1 = "pers_ping_collection_es";
	private String ESindex_collection = "pers_ping_collection_es";
	...
	public get_ping_elasticsearch_aggregate(MongoDB_Connector mongoConnector2,Elasticsearch_Connector eSConnector2) {
...
//get working sites list
		...
		for (Document document : documents) 
		{
			...
			if (document.getString("timestamp").contains(yesterday_timeStamp))//change it back to today timestamp
			 ...
			 try (InputStream in = ping_DailyreportConsumer.class.getResourceAsStream(path)) {
				 ...
				 for(int j=0; j<Number_of_sites; j++) {
					 ...
					 for(int k=0; k<anStringArray[Number_of_sites].length; k++) {
						 ...
						 else if( k!=0 && !anStringArray[j][k].equals("-")) //at k==0 we have the site name
						 {
...
// Here we are putting latency value in elastic search as index No, date, source site, destination, latency value
							 ESConnector.insertPingLatencyDataAggregate(ESindex,/*simpleDateFormat.format*/(yesterday()),sites_name[j].replace("-", "_"),sites_name[k-1].replace("-", "_"),(livliness_site));
						 }
			 ...
			 for (int l = 0; l < sites_name_working.length; l++)
			 {
				...
				/* }
				 ...
				 {*/
					...
					// Here we are putting ping collection in elastic search as index No, date, site, value
					 ESConnector.insertPingCollection(ESindex1,/*simpleDateFormat.format*/(yesterday()),sites_name_working[l].replace("-", "_"),Integer.parseInt((document_pingcollection.getString(sites_name_working[l]))));
	...
	private void calculate_ping(Document document, String microbox_site_source,String yesterday_timeStamp) throws IOException {
		...
		try (InputStream in = ping_DailyreportConsumer.class.getResourceAsStream(path)) {
			...
			for (Object o : itr) {
				...
				if (document.getString(microbox_site).contains("1"))
				{
					if (!microbox_ping_hmap.containsKey(microbox_site) || microbox_ping_hmap.get(microbox_site)==null)//Counting for the first time.
					{
						...
						anStringArray[count_site][count_site_vertical]= "1";/////////////////Error
					}
				...
				else if (document.getString(microbox_site).contains("0"))
				{
					if (!microbox_ping_hmap.containsKey(microbox_site) || microbox_ping_hmap.get(microbox_site)==null)//Counting for the first time.
					{
				...
				else if (document.getString(microbox_site).contains("-"))
				{
					microbox_ping_hmap.put(microbox_site,-1);
