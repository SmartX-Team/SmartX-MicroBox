...
public class get_ping_aggregate  extends TimerTask {
	...
	public get_ping_aggregate(MongoDB_Connector mongoConnector2) {
...
//get number of sites.
		...
		try {
			...
			SitAliveCount= new String[count_site_yaml+1][count_site_yaml+1];
			 ...
			 try (InputStream in = ping_DailyreportConsumer.class.getResourceAsStream(path)) {
				 ...
				 { 
					 ...
					 if (found!="TRUE")
					 {
						 ...
						 SitAliveCount[count_site][0]=microbox_site_source;
						
						 for(int j=1; j<count_site_yaml+1; j++) 
						 {
							 SitAliveCount[count_site][j]= "0";									
						 }
...
// get total ping for each site in Int array
					 ...
					 for(int k=0; k<SitAliveCount[Number_of_sites].length; k++) {
						 System.out.println("			===================================================");
						 System.out.println("			Values at arr["+j+"]["+k+"] is "+SitAliveCount[j][k]); //example Values at arr[0][0] is microbox-gist-1

						 if( SitAliveCount[j][k].equals(null)) //at k==- we have the site name
						 {
							 System.out.println("%%%%%%%%%%%%%%%%%% anStringArray[j][k].equals(null))");
						 }

						 else if( k!=0 && !SitAliveCount[j][k].equals("-")) //at k==- we have the site name
						 {
							 ...
							 System.out.println("Value of site"+sites_name[j]+" at SitAliveCount["+j+"]["+k+"]:		"+SitAliveCount[j][k]);
							 System.out.println("(Integer.valueOf(SitAliveCount["+j+"]["+k+"]):"+(Integer.valueOf(SitAliveCount[j][k])));


							 System.out.println("Percentage of site alive arr["+j+"]["+k+"] is 	"+((float)(Integer.valueOf(SitAliveCount[j][k]))/site_occourance)*100+"%");
							 if(site_occourance==0)
							 {
								 livliness_site=0;
							 }
							 else
								 livliness_site=round(((float)(Integer.valueOf(SitAliveCount[j][k]))/site_occourance)*100,2);
						 ...
						 else if( SitAliveCount[j][k].equals("-"))
						 {
...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	private void calculate_ping(Document document, String microbox_site_source,String yesterday_timeStamp) throws IOException {
		...
		try (InputStream in = ping_DailyreportConsumer.class.getResourceAsStream(path)) {
			...
			SitAliveCount[count_site][0]=microbox_site_source;
			System.out.println("Source_site="+SitAliveCount[count_site][0]);
			for (Object o : itr) {
				...
				if ((document.getString(microbox_site)!=null) && document.getString(microbox_site).contains("1"))
				{
					if (!microbox_ping_hmap.containsKey(microbox_site) || microbox_ping_hmap.get(microbox_site)==null)//Counting for the first time.
					{
						...
						System.out.println("anStringArray[count_site][count_site_vertical]="+SitAliveCount[count_site][count_site_vertical]);
						microbox_ping_hmap.put(microbox_site,1);
						SitAliveCount[count_site][count_site_vertical]= "1";
					}

					else
					{
						microbox_ping_hmap.put(microbox_site,microbox_ping_hmap.get(microbox_site)+1);
						SitAliveCount[count_site][count_site_vertical]= microbox_ping_hmap.get(microbox_site).toString();
					}
				...
				else if ((document.getString(microbox_site)!=null) && document.getString(microbox_site).contains("0"))
				{
					if (!microbox_ping_hmap.containsKey(microbox_site) || microbox_ping_hmap.get(microbox_site)==null)//Counting for the first time.
					{
						...
						SitAliveCount[count_site][count_site_vertical]= "0";
					}
				...
				{
					microbox_ping_hmap.put(microbox_site,-1);
					SitAliveCount[count_site][count_site_vertical]= "-";
				}
				...
				System.out.println("Site_Values="+SitAliveCount[count_site][count_site_vertical]);
