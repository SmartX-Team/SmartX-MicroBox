...
public class TenantVLANMapping implements Runnable{
		...
		try
        {
            ...
            while (true)
            {
                ...
                if (line!=null)
                {
                	...
                	tenantID    = line.substring(StringUtils.ordinalIndexOf(line, ",", 1)+1, StringUtils.ordinalIndexOf(line, ",", 2)).trim();
                	tenantName  = line.substring(StringUtils.ordinalIndexOf(line, ",", 2)+1, StringUtils.ordinalIndexOf(line, ",", 3)).trim();
                	userID      = "";
                	userName    = "";
	...
	public void run() 
	{
		while (true)
		{
	    	...
	    	deleteResult = db.getCollection(tenant_VLAN_MongoCollection).deleteMany(new Document());
